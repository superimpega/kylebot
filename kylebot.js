...
function f_foxbotInit() { // init foxbot, gets called at the very end
	window.setTimeout(function(){API.sendChat("/me Kylebot Online");}, 5000);
...
function f_commands(data) {
    ...
    //API.sendChat('/me Commands currently supported are: /commands, /rules, /cookie, /lock, /unlock, /skip, /retry, /rapes kyle, /hugs kyle, brb, /about, /autoskip, /joke, /test, /reload, /swag');
...
function f_skip(data) {
	...
	window.setTimeout(function(){API.sendChat("/me [kylebot] Your song got skipped because it was either not on genre, overplayed or (the outro) was too long.");}, 2000);
}
function f_long() {
	API.sendChat('@'+o_tmp.username+' [kylebot] Sorry, your song is over the allowed time limit.');
    window.setTimeout(function(){new ModerationForceSkipService(Models.room.data.historyID);}, 1000);
}
function f_lock(data) {
        API.sendChat('/me [kylebot] Dj Booth has been locked by operator!');
...
function f_unlock(data) {
        API.sendChat('/me [kylebot] Dj Booth has been unlocked by operator!');
        rpcGW.execute('room.update_options', null, Models.room.data.id,
              {
                name: Models.room.data.name,
                description: Models.room.data.description,
...
function f_retry(data) {
		API.sendChat('/me [kylebot] Please choose a different song and try again.');
...
function f_cookie(data) {
        ...
        window.setTimeout(function(){API.sendChat('/me [kylebot] tosses a cookie at '+data.from);}, 500);
}
...
function f_about(data) {
		API.sendChat('/me Hello, I am Kylebot. Coded by 1NT, Edited by Super');
}
function f_brb(data) {
